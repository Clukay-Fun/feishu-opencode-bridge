...
当前默认入库抽取由通用法律 core 负责，劳动法只作为可选领域能力示例。

本阶段不实现 skill runtime、不新增 plugin 注册、不新增配置项。后续如果补劳动法 skill，只优先作用于查询理解、回答组织、术语偏好和标签偏好，不改变默认入库镜像，避免同一份材料因不同 skill 产生不同入库结果。
