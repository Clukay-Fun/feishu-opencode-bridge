...
>
> 这份文档定义当前产品化阶段的架构基线。
> 如果它与旧阶段说明冲突，以这份文档的代码组织规则为准。

## 目标

框架在 seam 上冻结，功能在 seam 内扩展。

这个仓库当前定位为飞书原生 OpenCode runtime + 法律材料工作台。

`0.2.0` 的边界是：Bridge runtime seam 保持冻结，业务能力围绕案件工作台、知识库召回质量、设计器卡片和劳动二审断点继续扩展。
...
| 能力 | 分类 | `general` 默认 | `legal` 默认 | 边界规则 |
| :-- | :-- | :-- | :-- | :-- |
| Feishu OpenCode runtime、窗口/会话、队列、权限、过程卡片 | framework | 开 | 开 | 必须留在 core/runtime seam，不带法律语义 |
| 文件识别、材料上下文、document-pipeline、OCR provider 编排 | framework | 开 | 开 | 只提供解析与材料上下文，不决定领域动作 |
| 基础卡片、transport、formatter、成本统计、更新/备份/doctor | framework | 开 | 开 | 不依赖法律扩展 |
| memory | shared service | 开 | 开 | 只提供长期记忆能力；用户画像不得复用 `profile` 命名 |
| knowledge-base | shared service + legal preset | 关 | 开 | 基础设施可通用，但当前内置 prompt、命令和默认数据结构是法律版预设；通用版若启用必须显式配置 prompt / 表结构 |
| contract-assistant | business extension | 关 | 开 | 合同、发票、案件台账字段属于法律业务，不进 core |
| labor-skill | business extension | 关 | 开 | 劳动争议策略、证据链、二审复核属于法律业务 |
| case-workbench | business workspace | 关 | 开 | 案件工作台状态属于法律材料工作台，不作为通用 runtime 默认能力 |
| external extensions | extension seam | 可用 | 可用 | 外部扩展只能依赖 `src/extension-api/`，是否启用由 `extensions["id"].enabled` 决定 |

实现判定：

- 新能力如果不包含领域语义，优先进入 framework / shared service，例如文件解析、材料引用、通用记忆、通用 checklist、通用卡片原语。
- 新能力如果包含法律、劳动、合同、发票、案件、法条、法院、仲裁等语义，默认进入法律版扩展或法律 preset，不得作为 `general` 默认能力。
- `profile` 不是运行时分支条件。模块内部不应写 `if profile === "legal"`；需要差异时通过 extension `enabled`、模块配置、prompt override 或外部扩展实现。
- 通用版允许显式启用 `knowledge-base`，但此时应提供通用 prompt、通用表字段和通用命令文案；不得把法律默认 prompt 当作通用产品口径。
...
临时展示文案、一次性捷径、半连通命令，不应继续留在活跃产品路径里。
...
- 用户侧卡片以 `docs/cards/spec.md` 为当前样式真值；用户侧入口停用时，必须同步删除调用面、formatter export、测试断言和用户侧文档提及，避免保留孤儿卡片入口
