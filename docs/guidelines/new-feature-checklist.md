...
- `extension-api` 边界：`npm run lint:deps` 会把外部扩展目录限制到只能 import `src/extension-api/**`，不得直接 import bridge/runtime/feishu/store 或业务实现。
...
- `extension-api` 边界：面向外部扩展的新类型、helper 或 port 必须从 `src/extension-api/` 暴露；不要要求扩展 import 内部目录
...
- `formatter` 边界：新卡片应通过 family entrypoints（`shared-primitives`、`runtime-cards`、`knowledge-cards`、`labor-cards`、`contract-cards`）接入；业务展示卡默认走 `business template runtime + family adapter`，业务卡片模板通过 `extension.meta.ts` 注册，runtime 与业务模块不得直接依赖 `src/feishu/templates/*`；业务专属卡片只允许用于提醒类或流程类场景，普通查询、说明、一次性结果和错误提示应优先使用通用 notice / 通用结果卡
