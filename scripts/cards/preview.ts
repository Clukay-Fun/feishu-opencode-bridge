 ...
 * - 仅覆盖用户侧保留卡片，不发送已弃用的群聊和 Harness 独立报告卡。
 */
...
function main(): void {
  ...
  sendText(target, `卡片预览开始发送：共 ${cards.length} 张。当前不包含已弃用的群聊和 Harness 独立报告卡。`);
...
function createPreviewCards(): PreviewCard[] {
  ...
  return [
    ...
    card("运行时", "已完成", ["turn", "completed"], buildTurnStatusCardPayload({ title: "已完成", status: "完成", sessionId: "sess_abc123456789", durationText: "1m 12s", progressUpdates: [], toolUpdates: doneSteps, output: { text: "分析已完成，已生成摘要和后续建议。", paths: ["/tmp/sample-report.md"], commands: ["/status"] }, costSummary: "本次约 ¥0.042" })),
    ...
    card("合同/发票/案件", "案件已录入", ["case", "completed"], buildCaseCreateCompletedPayload(createCaseCreateResult(), sampleUrl, "委托人张三，对方某科技有限公司，案由违法解除劳动合同争议。")),
    card("合同/发票/案件", "案件待办", ["case", "todo", "reminder"], buildCaseTodoReminderPayload({ items: [
      { line: "（2026）沪01民初123号｜一审｜进行中\n日期：开庭日 2026-05-16；举证截止日 2026-05-12\n待办：补充工资流水证据", url: sampleUrl },
      { line: "张三 vs 某科技有限公司 劳动争议\n待办：联系当事人确认解除通知送达时间\n进展：证据清单待复核", url: sampleUrl },
    ] })),
    card("合同/发票/案件", "合同起草", ["contract", "draft"], buildContractDraftProgressPayload(contractDraftView)),
    card("合同/发票/案件", "合同起草完成", ["contract", "draft"], buildContractDraftCompletedPayload(contractDraftDoneView, { wordPath: "/Users/clukay/Documents/劳动合同解除协议.docx", recordId: "rec_contract_sample", warnings: ["建议人工复核竞业限制条款。"] }, { elapsedMs: 45000, recordUrl: sampleUrl })),
    ...
    card("合同/发票/案件", "发票识别完成", ["invoice"], buildInvoiceRecognizeCompletedPayload(createInvoiceRecognizeResult(), { elapsedMs: 32000, recordUrl: sampleUrl })),
    ...
    card("劳动分析", "二次审查完成", ["labor", "review"], buildLaborReviewCompletedPayload({ title: "张三诉某科技有限公司劳动争议", materialCount: 5, evidenceCount: 12, issueCount: 4, tagCounts, reviewStatus: "二审通过：法条引用和请求权基础一致。", findingsCount: 2, humanReviewCount: 1, docUrl: sampleUrl, ledgerUrl: sampleUrl, keyEvidenceViewUrl: sampleUrl, missingEvidenceViewUrl: sampleUrl, syncedEvidenceCount: 12, syncedGapCount: 3 })),
    ...
    card("知识库", "知识入库完成", ["knowledge", "completed"], buildKnowledgeIngestCompletedPayload({ completedCount: 4, failedCount: 1, queuedCount: 0, totalExtractedCount: 71, totalDedupedCount: 14, elapsedMs: 45000, bitableUrl: sampleUrl, results: createKnowledgeIngestResults(), failures: [{ sourceFile: "损坏文件.pdf", reason: "解析失败" }] })),
    ...
    card("知识库", "法律咨询", ["knowledge", "query"], buildKnowledgeQueryPayload(createKnowledgeQueryResult(sampleUrl))),
